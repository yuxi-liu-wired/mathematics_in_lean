...
theorem my_lemma4 :
  ...
  calc
    ...
    _ ≤ |x| * ε := by
      have h: |x| ≥ 0 := by
        apply abs_nonneg
      refine mul_le_mul_of_nonneg_left ?h h
      exact LT.lt.le ylt
    _ < ε := by
      calc
        |x| * ε < ε * ε := by
          exact (mul_lt_mul_right epos).mpr xlt
        _ ≤ ε := by
          exact (mul_le_iff_le_one_left epos).mpr ele1
